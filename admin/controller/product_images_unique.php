<?php
	...
	$web_id = isset($_REQUEST['web_id']) ? $_REQUEST['web_id'] : '0';
	...
	&& $imageFileType != "gif" ) {
		$objMain["msg"] = "Sorry, only JPG, JPEG, PNG & GIF files are allowed, ".$imageFileType." Not allowed.";
		$uploadOk = 0;
	}
			...
			$returnHTML = '
				<a>
					<div class="unlink_unsaved remove_uploaded_img" call_link="'.SERVER_PATH.'controller/controller.php?method=deleteunsavedimage-API&img='.$return_target_file.'" >X</div>
                  	<input type="hidden" name="aditionalImageUnique['.$web_id.'][]" id="" value="'.$return_target_file.'" >
