...
class TBL_PRODUCT_IMAGES {

	/* This function will be used to insert new entry in the table */
	public function insert($p_image_id,$product_id,$image_link,$description){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Insertion failed, Please try one more time");		 		 
		$insert_query = "INSERT INTO `tbl_product_images` (`p_image_id`,`product_id`,`image_link`,`description`,`created_on`,`updated_on`) 
					VALUES('$p_image_id','$product_id','$image_link','$description',now(),now());
				";
	...
	public function edit($p_image_id,$product_id,$image_link,$description){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Updation failed, Please try one more time");		 		 
		$update_query = "UPDATE `tbl_product_images` SET 
						`p_image_id`='$p_image_id',
						`product_id`='$product_id',
						`image_link`='$image_link',
						`description`='$description',
	...
	public function delete($p_image_id){
		...
		$update_query = "DELETE FROM `tbl_product_images` WHERE p_image_id = '$p_image_id'";
		$result = $mysqli->query($update_query); 			
		if($result){			
			$mainObj['status'] = true;
	...
	public function deleteproduct($p_id){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Deleting failed, Please try one more time");		 		 
		$update_query = "DELETE FROM `tbl_product_images` WHERE product_id = '$p_id'";
		$result = $mysqli->query($update_query); 			
		if($result){			
			$mainObj['status'] = true;
			$mainObj['msg'] = 'Deleted successfully';
	...
	public function getdetails($p_image_id){
		...
		$sql = "SELECT 
				...
				`created_on`,
				`updated_on`
			 FROM `tbl_product_images` WHERE p_image_id = '$p_image_id'";
	...
	public function productimages($product_id){ 
		...
		$sql = "SELECT 
				...
				`created_on`,
				`updated_on`
			 from `tbl_product_images` where product_id = '$product_id';";
		$result = $mysqli->query($sql);
		if ($result->num_rows > 0) {
			// output data of each row
			$i=0;
	...
	public function copyitem($new_product_id,$new_image_link, $p_image_id){
		...
		$insert_query = "INSERT INTO `tbl_product_images` (`product_id`,`image_link`,`description`,`created_on`,`updated_on`)
					SELECT 
				'$new_product_id',
				'$new_image_link',
				`description`,
