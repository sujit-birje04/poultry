...
class TBL_PRODUCT_SPECIAL_PROP {

	/* This function will be used to insert new entry in the table */
	public function insert($ps_id,$product_id,$name,$value,$sort_order,$is_active){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Insertion failed, Please try one more time");		 		 
		$insert_query = "INSERT INTO `tbl_product_special_prop` (`ps_id`,`product_id`,`name`,`value`,`sort_order`,`is_active`,`created_on`,`updated_on`) 
					VALUES('$ps_id','$product_id','$name','$value','$sort_order','$is_active',now(),now());
				";
	...
	public function edit($ps_id,$product_id,$name,$value,$sort_order,$is_active){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Updation failed, Please try one more time");		 		 
		$update_query = "UPDATE `tbl_product_special_prop` SET 
						`product_id`='$product_id',
						`name`='$name',
						`value`='$value',
						`sort_order`='$sort_order',
	...
	public function delete($ps_id){
		...
		$update_query = "DELETE FROM `tbl_product_special_prop` WHERE ps_id = '$ps_id'";
		$result = $mysqli->query($update_query); 			
		if($result){			
			$mainObj['status'] = true;
	...
	public function deleteproduct($p_id){
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array("status"=>false, "msg"=>"Deleting failed, Please try one more time");		 		 
		$update_query = "DELETE FROM `tbl_product_special_prop` WHERE product_id = '$p_id'";
		$result = $mysqli->query($update_query); 			
		if($result){			
			$mainObj['status'] = true;
			$mainObj['msg'] = 'Deleted successfully';
	...
	public function getdetails($ps_id){
		...
		$sql = "SELECT 
				...
				`created_on`,
				`updated_on`
			 FROM `tbl_product_special_prop` WHERE ps_id = '$ps_id'";
	...
	public function productspecials($product_id){ 
		$mysqli = Configuration::mysqli_configation();
		$mainObj = Array();
		$sql = "SELECT 
				`ps_id`,
				...
				`created_on`,
				`updated_on`
			 from `tbl_product_special_prop` where product_id = '$product_id';";
		$result = $mysqli->query($sql);
		if ($result->num_rows > 0) {
			// output data of each row
			$i=0;
	...
	public function copyitem($new_product_id, $ps_id){
		...
		$insert_query = "INSERT INTO `tbl_product_special_prop` (`product_id`,`name`,`value`,`sort_order`,`is_active`,`created_on`,`updated_on`)
					SELECT 
				'$new_product_id',
				`name`,
				`value`,
